...
fn day3_p1a() -> i32 {
    ...
    if let Ok(lines) = read_lines("src/day03/day03_input.txt") {
        let mut bitcount = 0;
        let mut d = Vec::new();
...
pub fn day03_p2a() {
    ...
    if let Ok(lines) = read_lines("src/data/day03_input.txt") {
        let mut bitcount = 0;
        let mut d = Vec::new();
...
pub fn day03_p2() {
    ...
    if let Ok(lines) = read_lines("src/data/day03_example.txt") {
        for line in lines {
            if let Ok(ip) = line {
                //let cur:i32 = i32::from_str(&ip).unwrap();
